...
class SoldProductListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Umumiy statistika
        total_cost = sum([sp.product.cost_price for sp in all_sold])
        total_revenue = sum([sp.sold_price for sp in all_sold])
        ...
        daily_cost = sum([sp.product.cost_price for sp in daily_sold])
        daily_revenue = sum([sp.sold_price for sp in daily_sold])
        context['daily_count'] = daily_sold.count()
        context['daily_cost'] = daily_cost
        context['daily_revenue'] = daily_revenue
        context['daily_profit'] = context['daily_revenue'] - context['daily_cost']

        # Haftalik statistika
        week_start = now - timedelta(days=7)
        weekly_sold = all_sold.filter(sold_at__gte=week_start)
        weekly_cost = sum([sp.product.cost_price for sp in weekly_sold])
        weekly_revenue = sum([sp.sold_price for sp in weekly_sold])
        context['weekly_count'] = weekly_sold.count()
        context['weekly_cost'] = weekly_cost
        context['weekly_revenue'] = weekly_revenue
        context['weekly_profit'] = context['weekly_revenue'] - context['weekly_cost']

        # Oylik statistika
        month_start = now - timedelta(days=30)
        monthly_sold = all_sold.filter(sold_at__gte=month_start)
        monthly_cost = sum([sp.product.cost_price for sp in monthly_sold])
        monthly_revenue = sum([sp.sold_price for sp in monthly_sold])
        context['monthly_count'] = monthly_sold.count()
        context['monthly_cost'] = monthly_cost
        context['monthly_revenue'] = monthly_revenue
        context['monthly_profit'] = context['monthly_revenue'] - context['monthly_cost']
